...
from gcloudoem import KeyProperty, connect
from gcloudoem.datastore.connection import disconnect
...
class TestConnection(unittest2.TestCase):
    ...
    def _getTargetClass(self):
        from gcloudoem.datastore.connection import Connection
    ...
    def _make_query_pb(self, kind):
        from gcloudoem.datastore.connection import datastore_pb
    ...
    def _verifyProtobufCall(self, called_with, URI, conn):
        ...
        self.assertEqual(called_with['headers']['Content-Type'], 'application/x-protobuf')
        self.assertEqual(called_with['headers']['User-Agent'], conn.USER_AGENT)

    def test_custom_url_from_env(self):
        import os
        from gcloudoem.datastore.connection import _GCD_HOST_ENV_VAR_NAME
        ...
        with patch.object(os, 'getenv', fake_environ.get):
            ...
            self.assertNotEqual(conn.api_base_url, conn.API_BASE_URL)
    ...
    def test__request_w_200(self):
        ...
        self.assertEqual(conn._request(METHOD, DATA), 'CONTENT')
    ...
    def test__request_not_200(self):
        from gcloudoem.exceptions import BadRequest
    ...
    def test__rpc(self):
        ...
        http = conn._http = Http({'status': '200'}, 'CONTENT')
        response = conn._rpc(METHOD, ReqPB(), RspPB)
    ...
    def test_lookup_single_key_empty_response(self):
        ...
        found, missing, deferred = conn.lookup([key_pb])
    ...
    def test_lookup_single_key_empty_response_w_eventual(self):
        ...
        http = conn._http = Http({'status': '200'}, rsp_pb.SerializeToString())
        found, missing, deferred = conn.lookup([key_pb], eventual=True)
        self.assertEqual(len(found), 0)
        self.assertEqual(len(missing), 0)
        self.assertEqual(len(deferred), 0)
        cw = http._called_with
        ...
        self.assertEqual(len(keys), 1)
        _compare_key_pb_after_request(self, key_pb, keys[0])
        self.assertEqual(request.read_options.read_consistency, datastore_pb.ReadOptions.EVENTUAL)
    ...
    def test_lookup_single_key_empty_response_w_eventual_and_transaction(self):
        ...
        key_pb = self._make_key_pb()
        conn = self._makeOne()
        self.assertRaises(ValueError, conn.lookup, key_pb, eventual=True, transaction_id=TRANSACTION)
    ...
    def test_lookup_single_key_empty_response_w_transaction(self):
        ...
        http = conn._http = Http({'status': '200'}, rsp_pb.SerializeToString())
        found, missing, deferred = conn.lookup([key_pb], transaction_id=TRANSACTION)
        self.assertEqual(len(found), 0)
        self.assertEqual(len(missing), 0)
        self.assertEqual(len(deferred), 0)
        cw = http._called_with
    ...
    def test_lookup_multiple_keys_empty_response(self):
        ...
        key_pb1 = self._make_key_pb()
        key_pb2 = self._make_key_pb(id=2345)
        rsp_pb = datastore_pb.LookupResponse()
        conn = self._makeOne()
        URI = '/'.join([
            conn.api_base_url,
        ...
        http = conn._http = Http({'status': '200'}, rsp_pb.SerializeToString())
        found, missing, deferred = conn.lookup([key_pb1, key_pb2])
        self.assertEqual(len(found), 0)
        self.assertEqual(len(missing), 0)
        self.assertEqual(len(deferred), 0)
        cw = http._called_with
    ...
    def test_lookup_multiple_keys_w_missing(self):
        ...
        http = conn._http = Http({'status': '200'}, rsp_pb.SerializeToString())
        result, missing, deferred = conn.lookup([key_pb1, key_pb2])
        self.assertEqual(result, [])
        self.assertEqual(len(deferred), 0)
        self.assertEqual([missed.key for missed in missing], [key_pb1, key_pb2])
        cw = http._called_with
        self._verifyProtobufCall(cw, URI, conn)
        rq_class = datastore_pb.LookupRequest
        request = rq_class()
    ...
    def test_lookup_multiple_keys_w_deferred(self):
        ...
        self.assertEqual(cw['headers']['Content-Type'], 'application/x-protobuf')
    ...
    def test_run_query_w_eventual_no_transaction(self):
        ...
        http = conn._http = Http({'status': '200'}, rsp_pb.SerializeToString())
        pbs, end, more, skipped = conn.run_query(q_pb, eventual=True)
    ...
    def test_run_query_wo_eventual_w_transaction(self):
        ...
        http = conn._http = Http({'status': '200'}, rsp_pb.SerializeToString())
        pbs, end, more, skipped = conn.run_query(q_pb, transaction_id=TRANSACTION)
        self.assertEqual(pbs, [])
        self.assertEqual(end, CURSOR)
        self.assertTrue(more)
        self.assertEqual(skipped, 0)
        ...
        self.assertEqual(request.partition_id.namespace, '')
        self.assertEqual(request.query, q_pb)
        self.assertEqual(request.read_options.read_consistency, datastore_pb.ReadOptions.DEFAULT)
        self.assertEqual(request.read_options.transaction, TRANSACTION)
    ...
    def test_run_query_w_eventual_and_transaction(self):
        ...
        rsp_pb.batch.entity_result_type = datastore_pb.EntityResult.FULL
        conn = self._makeOne()
        self.assertRaises(ValueError, conn.run_query, q_pb, eventual=True, transaction_id=TRANSACTION)
    ...
    def test_begin_transaction_default_serialize(self):
        ...
        http = conn._http = Http({'status': '200'}, rsp_pb.SerializeToString())
        self.assertEqual(conn.begin_transaction(), TRANSACTION)
        cw = http._called_with
        self._verifyProtobufCall(cw, URI, conn)
    ...
    def test_begin_transaction_explicit_serialize(self):
        ...
        http = conn._http = Http({'status': '200'}, rsp_pb.SerializeToString())
        self.assertEqual(conn.begin_transaction(True), TRANSACTION)
        cw = http._called_with
        self._verifyProtobufCall(cw, URI, conn)
        rq_class = datastore_pb.BeginTransactionRequest
        request = rq_class()
    ...
    def test_commit_w_transaction(self):
        ...
        http = conn._http = Http({'status': '200'}, rsp_pb.SerializeToString())
        result = conn.commit(mutation, b'xact')
        self.assertEqual(result.index_updates, 0)
        self.assertEqual(list(result.insert_auto_id_key), [])
        cw = http._called_with
        self._verifyProtobufCall(cw, URI, conn)
    ...
    def test_rollback_ok(self):
        ...
        http = conn._http = Http({'status': '200'}, rsp_pb.SerializeToString())
        self.assertEqual(conn.rollback(TRANSACTION), None)
        cw = http._called_with
        self._verifyProtobufCall(cw, URI, conn)
    ...
    def test_allocate_ids_empty(self):
        ...
        http = conn._http = Http({'status': '200'}, rsp_pb.SerializeToString())
        self.assertEqual(conn.allocate_ids([]), [])
        cw = http._called_with
        self._verifyProtobufCall(cw, URI, conn)
    ...
    def test_allocate_ids_non_empty(self):
        ...
        before_key_pbs = [self._make_key_pb(id=None), self._make_key_pb(id=None)]
        after_key_pbs = [self._make_key_pb(), self._make_key_pb(id=2345)]
        rsp_pb = datastore_pb.AllocateIdsResponse()
        ...
        http = conn._http = Http({'status': '200'}, rsp_pb.SerializeToString())
        self.assertEqual(conn.allocate_ids(before_key_pbs), after_key_pbs)
        cw = http._called_with
        self._verifyProtobufCall(cw, URI, conn)
        rq_class = datastore_pb.AllocateIdsRequest
        request = rq_class()
...
def _compare_key_pb_after_request(test, key_before, key_after):
    ...
    test.assertEqual(key_before.partition_id.namespace, key_after.partition_id.namespace)
